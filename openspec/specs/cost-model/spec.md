...
## Purpose
Defines `CostDef` as a first-class type, the `assert` built-in keyword for affordability guards, how costs are declared on `CardDefinition` and `NamedEffectBlockDef`, how they are paid during action execution, and how they are validated via a lightweight state clone.
